...
exports.initializeExcelFile = initializeExcelFile;
exports.appendBusinessToExcel = appendBusinessToExcel;
exports.exportBusinessesToExcel = exportBusinessesToExcel;
...
function isUsefulTextField(businesses, selector) {
    let anyValue = false;
...
function buildManagementStrings(biz) {
    const contacts = (biz.contactPersons ?? []);
    const formatPerson = (p) => {
        const parts = [];
        if (p.name)
            parts.push(p.name);
        if (p.personalId)
            parts.push(`ID: ${p.personalId}`);
        if (p.phone)
    ...
    const findByRole = (keyword) => {
        const person = contacts.find((p) => (p.position ?? "").toLowerCase().includes(keyword.toLowerCase()));
...
function getStandardColumns() {
    return [
        ...
        { header: "Social_Responsibility", key: "socialResponsibility", width: 28 },
        { header: "Mobile_Service", key: "mobileService", width: 24 },
        { header: "Internet_Service", key: "internetService", width: 28 },
        { header: "Oil_Companies", key: "oilCompanies", width: 32 },
        { header: "Banks", key: "banks", width: 40 },
        { header: "Insurance", key: "insurance", width: 40 },
        { header: "Export_Info", key: "exportInfo", width: 32 },
        { header: "Import_Info", key: "importInfo", width: 36 },
        { header: "Local_Shipments", key: "localShipments", width: 32 },
        { header: "International_Shipments", key: "internationalShipments", width: 36 },
        { header: "Local_Partners", key: "localPartners", width: 32 },
        { header: "Foreign_Partners", key: "foreignPartners", width: 36 },
        { header: "Local_Suppliers", key: "localSuppliers", width: 32 },
        { header: "Foreign_Suppliers", key: "foreignSuppliers", width: 36 },
        { header: "Local_Distributors", key: "localDistributors", width: 32 },
        { header: "Local_Dealers", key: "localDealers", width: 32 },
        { header: "Audit_Service", key: "auditService", width: 28 },
        { header: "Legal_Service", key: "legalService", width: 28 },
        { header: "Accounting_Service", key: "accountingService", width: 28 },
        { header: "Consulting_Service", key: "consultingService", width: 28 },
        { header: "Advertising_Service", key: "advertisingService", width: 28 },
        { header: "Courier_Service", key: "courierService", width: 28 },
        { header: "Property_Valuation_Service", key: "propertyValuationService", width: 32 },
        { header: "Profile_URL", key: "profileUrl", width: 40 }
    ];
}
const georgianHeaderByEnglish = {
...
function formatBusinessRow(biz) {
    ...
    return {
        ...
        isVATPayer: biz.isVATPayer === null ? undefined : biz.isVATPayer ? "Yes" : "No",
        avgEmployeeAge: biz.avgEmployeeAge ?? undefined,
        genderMale: biz.genderDistribution?.male !== undefined && biz.genderDistribution?.male !== null
            ? `${biz.genderDistribution.male}%`
            : undefined,
        genderFemale: biz.genderDistribution?.female !== undefined && biz.genderDistribution?.female !== null
            ? `${biz.genderDistribution.female}%`
            : undefined,
...
async function initializeExcelFile(filePath) {
    if (workbook && worksheet && outputPath === filePath) {
        logger_1.logger.warn(`Excel file already initialized at ${filePath}`);
        return;
    }
    ...
    logger_1.logger.info(`Excel file initialized at ${filePath}`);
}
async function appendBusinessToExcel(biz) {
    if (!workbook || !worksheet || !outputPath) {
        throw new Error("Excel file not initialized. Call initializeExcelFile() first.");
    }
...
async function exportBusinessesToExcel(businesses, outputPath) {
    if (businesses.length === 0) {
        logger_1.logger.warn("No businesses to export, skipping Excel generation.");
        return;
    }
    ...
    await workbook.xlsx.writeFile(outputPath);
    logger_1.logger.info(`Excel file written to ${outputPath}`);
}
